...
- 如果用户要求画图但没有说明具体画什么，先停下来确认图形需求
- 如果任务同时包含敏感性分析和画图，默认拆成求解、数据保存、绘图三个文件，而不是放在一个大脚本里
...
如果你希望 Skill 帮你画图，最好直接说明要画哪些量。若你只说“把结果画出来”，Skill 应先停下来确认图形内容，再继续生成绘图代码。
...
7. Skill 先决定交付物是单文件还是拆成求解、数据、绘图多文件结构。
8. Skill 选择最接近的 FinHJB 模板。
9. Skill 生成代码。
10. Skill 运行测试闭环，并在必要时先修复再交付。
11. Skill 输出：
   - 结构化模型规格
   - 可执行的 FinHJB 代码
   - 已执行测试与修复摘要
...
- 如果任务同时包含敏感性分析、数据保存和画图，至少拆成三个文件：
  - 求解/模型文件
  - 数据保存或数据导出文件
  - 绘图文件
...
- 求解文件负责定义模型和跑 continuation 或 solve
- 数据文件负责把后续画图需要的结果保存下来
- 绘图文件负责读取数据并生成图形

这不是形式主义，而是为了让重跑求解、修改图形和检查诊断时都更清楚。
...
Skill 应该把交付物重构成分开的求解文件、数据文件和绘图文件。对于这种工作流，默认单文件并不清晰。
