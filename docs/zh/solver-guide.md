...
这一页是 package 路径、BCW 路径和 Model Coder 路径共用的工作流参考页。

如果你是在直接建模，请在 [建模指南](./modeling-guide.md) 后阅读；如果你还在仓库示例上建立直觉，请在 [快速开始](./getting-started.md) 后阅读。

如果你现在的问题已经变成“命令能跑，但结果不可信”，请优先看 [排障](./troubleshooting.md) 而不是继续做工作流选择。
